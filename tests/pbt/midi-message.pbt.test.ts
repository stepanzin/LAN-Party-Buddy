...
import { isValidMidiCC, parseMidiCC } from '@domain/midi-message';
...
describe('PBT: MIDI Message', () => {
  ...
  it('isValidMidiCC is consistent with parseMidiCC', () => {
    fc.assert(
      fc.property(fc.array(fc.integer({ min: -10, max: 300 }), { minLength: 0, maxLength: 5 }), (msg) => {
  ...
  it('rejects messages with wrong length', () => {
    fc.assert(
      fc.property(
        fc.array(fc.integer({ min: 0, max: 255 }), { minLength: 0, maxLength: 10 }).filter((arr) => arr.length !== 3),
        (msg) => {
